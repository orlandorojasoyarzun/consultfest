...
<head>
    ...
    <title>Crear cuenta - Consultfest</title>
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @else
                        ...
                        <a href="{{ route('auth.register') }}" class="text-sm text-[var(--text-primary)] font-medium">Registrarse</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="w-full max-w-md">
            <div class="text-center mb-10">
                <h1 class="text-3xl md:text-4xl font-semibold tracking-tight mb-3 cinema-fade-up cinema-stagger-1">
                    <span class="font-display italic font-normal">Crea tu cuenta</span>
                </h1>
                <p class="text-sm text-[var(--text-muted)] cinema-fade-up cinema-stagger-2">
                Empieza a recibir alertas sobre festivales que podrían interesar a tus producciones.
                </p>
            </div>
            ...
            <form action="{{ route('auth.register.process') }}" method="POST" class="space-y-4 cinema-fade-up cinema-stagger-5">
                @csrf

                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label for="name" class="block text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">
                            Nombre
                        </label>
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus maxlength="255"
                            class="cinema-input w-full px-4 py-3 text-sm @error('name') cinema-input-error @enderror"
                    ...
                    <div>
                        <label for="last_name" class="block text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">
                            Apellido
                        </label>
                        <input type="text" id="last_name" name="last_name" value="{{ old('last_name') }}" maxlength="255"
                            class="cinema-input w-full px-4 py-3 text-sm @error('last_name') cinema-input-error @enderror"
                ...
                <div>
                    <label for="production_company" class="block text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">
                        Productora
                    </label>
                    <input type="text" id="production_company" name="production_company" value="{{ old('production_company') }}" maxlength="255"
                        class="cinema-input w-full px-4 py-3 text-sm @error('production_company') cinema-input-error @enderror"
                        placeholder="Opcional — nombre de tu productora">
            ...
            <p class="text-center text-sm text-[var(--text-muted)] mt-8 cinema-fade-up cinema-stagger-7">
                ¿Ya tienes cuenta?
                <a href="{{ route('auth.login') }}" class="text-[var(--accent)] hover:underline transition-colors font-medium">Inicia sesión →</a>
            </p>
        </div>
    </main>
